...
//
// Version: see library.properties EPD2_4G_4G.h
//
...
//
// Example is extended by Mikhail Lavrenov to show anti aliased fonts on 4-grey EPD.
//

// base class GxEPD2_GFX can be used to pass references or pointers to the display instance as parameter, uses ~1.2k more code
// enable or disable GxEPD2_GFX base class
#define ENABLE_GxEPD2_GFX 1
//#define ENABLE_GxEPD2_4G_GFX 1  // this one forces to use another base GFX library instead of Adafruit_GFX. Not compatible with AA extention from M.Lavrenov
...
#include "GxEPD2_4G_MixedExample_ext.h"  // updated original file by Jean-Marc Zingg - added AA-font extension
...
void setup()
{
  ...
  helloFourGreyLevels();
  Serial.println("hello 4 gray levels (7s)");
  delay(7000);
  // code added by M.Lavrenov to clean up AntiAlisased text
  ...
  delay(1000);
  // end of code added by M.Lavrenov
  //return;
  helloArduino();
  Serial.println("Hello Arduino");
  delay(2000);
  helloEpaper();
  Serial.println("Hello epaper");
  delay(2000);
  //helloValue(123.9, 1);
  //delay(1000);
  showFont("FreeMonoBold9pt7b", &FreeMonoBold9pt7b);
  Serial.println("ShowFont FreeMonoBold9pt7b for 7 seconds");
  delay(7000);
  Serial.println("drawBitmaps started");
  drawBitmaps();
  Serial.println("drawBitmaps done");
  delay(2000);
  showGreyLevels();
  Serial.println("Show graylevels");
  if ((display.epd2.hasPartialUpdate) || true)
  {
    showPartialUpdate();
    Serial.println("ShowPartialUpdate");
    delay(2000);
  ...
  display.powerOff();
  deepSleepTest();
  Serial.println("setup() done, power off and deep sleep for EPD");
}
...
void helloFourGreyLevels()
{
  ...
  do
  {
    ...
    // following lines are added by M.Lavrenov for Anti Aliaced font
    display.setCursor(1,10); // no need to have a special cursor function for AA fonts
    display.setAAFont( OpenSans12 ); // doesn't affect display.print commands, they use a different font object
    display.printAA("AntiAliased font - OpenSans12.");
    //display.drawAAFontChar('W');
    display.setAAFont( OpenSans14 ); // doesn't affect display.print commands, they use a different font object
    display.setCursor(1,26); // no need to have a special cursor function for AA fonts
    display.printAA("AntiAliased font size 14.");
    display.setCursor(1,44); 
    display.printAA("Lorem ipsum dolor sit amet");
    /// end of AA font demo
...
void deepSleepTest()
{
  ...
  do
  {
    ...
    display.print(again);
    // code added by M.Lavrenov
    display.setAAFont( OpenSans14 ); // doesn't affect display.print commands, they use a different font object
    display.setCursor(ax-50,ay+20); // no need to have a special cursor function for AA fonts
    display.printAA("AntiAliased text");
    //display.setFont(&FreeMonoBold9pt7b); // return back - not required as printAA uses different command
    // end of code added by M.Lavrenov
  }
  while (display.nextPage());
  display.hibernate();
...
{
  //Serial.println("helloWorld");
  display_bw.setRotation(1);
  display_bw.setFont(&FreeMonoBold9pt7b);
  display_bw.setTextColor(GxEPD_BLACK);
  int16_t tbx, tby; uint16_t tbw, tbh;
  display_bw.getTextBounds(HelloWorld, 0, 0, &tbx, &tby, &tbw, &tbh);
  // center bounding box by transposition of origin:
  uint16_t x = ((display_bw.width() - tbw) / 2) - tbx;
  uint16_t y = ((display_bw.height() - tbh) / 2) - tby;
  display_bw.setFullWindow();
  display_bw.firstPage();
  do
  {
    display_bw.fillScreen(GxEPD_WHITE);
    display_bw.setCursor(x, y);
    display_bw.print(HelloWorld);
  }
...
void showPartialUpdate()
{
  ...
  for (uint16_t r = 0; r < 4; r++)
  {
    display_bw.setRotation(r);
    display_bw.setPartialWindow(box_x, box_y, box_w, box_h);
    display_bw.firstPage();
    do
    {
      display_bw.fillRect(box_x, box_y, box_w, box_h, GxEPD_BLACK);
      //display_bw.fillScreen(GxEPD_BLACK);
    }
    while (display_bw.nextPage());
    delay(2000);
    display_bw.firstPage();
    do
    {
      display_bw.fillRect(box_x, box_y, box_w, box_h, GxEPD_WHITE);
    }
    while (display_bw.nextPage());
    delay(1000);
  }
  ...
  for (uint16_t r = 0; r < 4; r++)
  {
    ...
    for (uint16_t i = 1; i <= 10; i += incr)
    {
      ...
      {
        display_bw.fillRect(box_x, box_y, box_w, box_h, GxEPD_WHITE);
        display_bw.setCursor(box_x, cursor_y);
        display_bw.print(value * i, 2);
      }
      while (display_bw.nextPage());
      delay(500);
    }
